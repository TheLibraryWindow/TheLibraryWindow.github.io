...
func _render_level_cards() -> void:
	...
	for level in ProgressManager.LEVELS:
		...
		var tagline := level_payloads[level].get("tagline", "")
		...
		(card as LevelView).configure(level, tagline, completion, breakdown)
		(card as LevelView).open_level.connect(func(code: String): _show_level(code))
