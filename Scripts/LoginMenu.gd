...
func _apply_responsive_layout() -> void:
	...
	var ui_scale := clampf(minf(width_ratio, height_ratio), 0.85, PANEL_MAX_SCALE)

	var panel_size := PANEL_BASE_SIZE * Vector2(ui_scale, ui_scale)
	...
	var text_scale := clampf(ui_scale, 0.9, 1.2)
...
func _adjust_layout_margins(viewport_size: Vector2) -> void:
	...
	layout_container.add_theme_constant_override("margin_left", roundi(horizontal_margin))
	layout_container.add_theme_constant_override("margin_right", roundi(horizontal_margin))
...
func _setup_visual_effects() -> void:
	# Add flowing particles background
	call_deferred("_add_background_effects")
	
	# Fade in the panel
	if is_instance_valid($Layout/Panel):
		call_deferred("_fade_in_panel")

func _add_background_effects() -> void:
	VisualEffects.create_flowing_particles(self, 8)
	var gradient_colors := [
		Color(0.05098, 0.070588, 0.12549, 1.0),
		Color(0.055, 0.08, 0.14, 1.0),
		Color(0.045, 0.09, 0.15, 1.0),
		Color(0.05098, 0.070588, 0.12549, 1.0)
	]
	VisualEffects.create_animated_gradient($Background, gradient_colors, 25.0)

func _fade_in_panel() -> void:
	if is_instance_valid($Layout/Panel):
		VisualEffects.smooth_fade_in($Layout/Panel, 0.6)
