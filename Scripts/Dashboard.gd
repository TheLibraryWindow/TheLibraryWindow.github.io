...
func _on_profile_changed(profile: Dictionary) -> void:
	...
	var current_level: String = profile.get("current_level", "A1")
...
func _refresh_skill_bars(level: String) -> void:
	for skill in skill_bars.keys():
		...
		var bar := skill_bars[skill] as ProgressBar
		VisualEffects.animate_progress_bar(bar, pct, 1.2)
...
func _show_section(section: String) -> void:
	...
	var target_section: Control
	match section:
		"dashboard":
			target_section = home_section
		"courses":
			target_section = courses_section
			_ensure_courses_view()
		"progress":
			target_section = progress_section
		"profile":
			target_section = profile_section
		"lesson":
			target_section = lesson_section
	
	if target_section:
		target_section.visible = true
		target_section.modulate.a = 0.0
		VisualEffects.smooth_fade_in(target_section, 0.4)
...
func _on_lesson_completed(payload: Dictionary) -> void:
	...
	var skill: String = payload.get("skill", "")
	var lesson_id: String = payload.get("lesson_id", "")
	var correct: int = payload.get("correct", 0)
	var attempted: int = payload.get("attempted", 0)
...
func _on_continue_pressed() -> void:
	...
	if last.get("lesson_id", "").is_empty():
		_show_section("courses")
		return
	var lesson_data: Dictionary = lesson_loader.find_lesson(last.get("level", ""), last.get("lesson_id", ""))
	if lesson_data.is_empty():
		_show_section("courses")
		return
	var skill: String = lesson_data.get("skill", last.get("skill", "Listening"))
...
func _setup_visual_effects() -> void:
	...
	var gradient_colors := [
		...
		Color(0.04, 0.09, 0.14, 1.0),
		Color(0.05, 0.07, 0.11, 1.0)
	]
	VisualEffects.create_animated_gradient(self, gradient_colors, 20.0)
	
	# Add floating orbs
	call_deferred("_add_floating_orbs")
	
	# Add smooth fade-in for main content
	call_deferred("_fade_in_content")

func _add_floating_orbs() -> void:
	for i in range(3):
		var orb := VisualEffects.create_orb_animation(self, 120.0 + randf() * 80.0)
		orb.position = Vector2(randf() * size.x, randf() * size.y)

func _fade_in_content() -> void:
	if is_instance_valid($Main):
		VisualEffects.smooth_fade_in($Main, 0.8)
